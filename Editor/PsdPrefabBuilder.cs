...
namespace Psd2Prefab
{
    ...
    public static class PsdPrefabBuilder
    {
        ...
        /// <param name="useTmp">文本图层是否生成 TextMeshProUGUI(需项目中存在 TMP,Unity 6 默认内置)</param>
        public static string Build(string psdPath, string outputFolder, bool useTmp = false)
        {
            _useTmp = useTmp;
#if PSD2PREFAB_TMP
            if (_useTmp && TMP_Settings.defaultFontAsset == null)
            {
                Debug.LogWarning("[PSD2Prefab] 未找到 TMP 默认字体资源(请先导入 TMP Essentials),本次回退为 Legacy Text。");
                _useTmp = false;
            }
#else
            _useTmp = false;
            ...
            Dictionary<string, int> usedNames)
        {
            ...
            string content = string.IsNullOrEmpty(layer.TextContent) ? layer.Name : layer.TextContent;
            var color = new Color(0f, 0f, 0f, layer.Opacity / 255f);
            bool isButton = HasPrefix(layer.Name, "btn_");
            Graphic graphic;
            ...
            if (_useTmp)
            {
                ...
                tmp.fontSizeMin = 8;
                tmp.fontSizeMax = Mathf.Max(10, layer.Height);
                tmp.color = color;
                tmp.raycastTarget = isButton;
                graphic = tmp;
            }
            else
#endif
